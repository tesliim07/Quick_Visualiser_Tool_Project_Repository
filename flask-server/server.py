...
def revertToOriginalDataset(fileName):
    try:
        deleteModifiedDataset = delete_modified_dataset(fileName)
        return deleteModifiedDataset, 200
    except Exception as e:
        return f'An error occurred while reverting to the original dataset: {str(e)}', 500
    
@app.route('/removeFile/<string:fileName>', methods=['DELETE'])
def removeFile(fileName):
    try:
        removeFile = delete_file_from_list(fileName)
        return removeFile, 200
    except Exception as e:
        return f'An error occurred while removing the file: {str(e)}', 500
