...
def process_file_for_box_plots(file_path, subfolder_path):
    ...
    df_melted = numerical_df.melt(var_name="Variable", value_name="Value")
    ...
    html_file_path = os.path.join(subfolder_path,"box_plot.html")
...
def generate_box_plot_url(fileName):
    modified_file_path = os.path.join(modified_folder, f'modified_{fileName}.csv')
    uploaded_file_path = os.path.join(uploaded_folder, f'{fileName}.csv')
    subfolder_path = os.path.join(box_plots_dir, f'boxPlot_{fileName}')
    if not os.path.exists(subfolder_path):
        os.makedirs(subfolder_path)
    if modified_file_path in modified_files:
        app.logger.info(f'Using modified file for boxPlots: {modified_file_path}')
